...
/**
 * Determine total each country's total cases that ISO week by adding distribution of epi week new cases.
...
function update_cumulative_cases(isoweek_hash) {
  ...
  Object.keys(isoweek_hash).forEach(d => {
    ...
    Object.keys(isoweek_hash[d]).forEach(c => {
      // Initialize a hash
      isoweek_hash[d][c].cumulative = {};
      var country_stats = isoweek_hash[d][c];
      pre_average = case_kinds.reduce((h, kind) => {
        // dates
        h[kind] = Object.keys(isoweek_hash[d][c].epi_week_2_cumulative[kind]).reduce((n, yyyymmdd) => {
          n += (country_stats.epi_weeks[yyyymmdd] * country_stats.epi_week_2_cumulative[kind][yyyymmdd]);
          return n;
        }, 0);
        return h;
      }, {})
      ...
      var num_days_in_epi_week = Object.keys(isoweek_hash[d][c].epi_weeks).reduce((n, yyyymmdd) => {
        n += country_stats.epi_weeks[yyyymmdd];
        return n;
      }, 0);
      case_kinds.forEach(kind => {
        isoweek_hash[d][c].cumulative[kind] = pre_average[kind] / num_days_in_epi_week;
      })
      // Data science requests imported_and_autochthonous_cases_confirmed figure.
        isoweek_hash[d][c].cumulative.imported_and_autochthonous_cases_confirmed =
        isoweek_hash[d][c].cumulative.autochthonous_cases_confirmed +
        isoweek_hash[d][c].cumulative.imported_cases;
    })
  })
 ...
 * @param{Object} daily_country_cases_hash - keys are daily dates, values are array of countries with that day's stats
 * @return{Object} Returns hash with iso_dates as keys
 */
 ...
 */
function add_epi_week_composition(isow_hash, isoweek_start_date, date_country_cases_hash, date) {
  ...
  Object.keys(date_country_cases_hash).forEach(c => {
    ...
    isow_start_day[c] = isow_start_day[c] || {
      new_cases_this_week: {},
      epi_weeks: {},
      epi_week_2_cases_per_day: {},
      epi_week_2_cumulative: {},
    };
    ...
    case_kinds.forEach(kind => {
      ...
      var new_cases_per_day = country_obj.new_cases_per_day[kind];

      // Cumulative confirmed cases for this epi week.
      var epi_cases_cumulative = country_obj.cases_cumulative[kind];
      // If first record of iso week, set to 0.
      isow_start_day[c].new_cases_this_week[kind] = isow_start_day[c].new_cases_this_week[kind] || 0;

      isow_start_day[c].new_cases_this_week[kind] += new_cases_per_day;

      // epi_weeks is a hash of epi week to number of times seen in this ISO week
      if (isow_start_day[c].epi_week_2_cases_per_day[kind]) {
        isow_start_day[c].epi_week_2_cases_per_day[kind][epi_week_date] = new_cases_per_day;

        isow_start_day[c].epi_week_2_cumulative[kind][epi_week_date] = epi_cases_cumulative;

      } else {
        isow_start_day[c].epi_week_2_cumulative[kind] = {};
        isow_start_day[c].epi_week_2_cumulative[kind][epi_week_date] = epi_cases_cumulative;

        isow_start_day[c].epi_week_2_cases_per_day[kind] = {};
        isow_start_day[c].epi_week_2_cases_per_day[kind][epi_week_date] = new_cases_per_day;
      }
    })
  })
...
function determine_new_cases_per_epi_week_date(filename_date, i, ordered_dates, all_dates_hash, dir_epi, provider) {
  return new Promise((resolve, reject) => {
    // Ignore first (earliest) file for now.
 ...
 * @return{hash} Returns iso_date_hash updated with epi_weeks object:
 */
  ...
  return Object.keys(obj_current.countries).reduce((h, c) => {
    ...
    case_kinds.forEach(kind => {
      ...
      if (cases_new < 0) {
        ...
        console.error('Found negative new', kind, 'cases for', c, 'on', filename_date, cases_new);
        cases_new = 0;
      }
      // Divide total new cases by number of days since last file date name.
      var new_cases_per_day = cases_new / diff_in_days;
      // Cases to date
      h[c].cases_cumulative[kind] = cases_cumulative;
      h[c].new_cases_per_day[kind] = new_cases_per_day; //Math.round(new_cases_per_day) //Math.ceil(new_cases_per_day * 100) / 100,
    })
    return h;
  }, {});
}
