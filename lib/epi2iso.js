...
var case_kind_lookup = {
  autochthonous: 'autochthonous_cases_confirmed',
...
function update_cumulative_cases(isoweek_hash) {
  Object.keys(isoweek_hash).forEach(d => {
    Object.keys(isoweek_hash[d]).forEach(c => {
      ...
      pre_average = ['autochthonous', 'imported'].reduce((h, kind) => {
      ...
      isoweek_hash[d][c].cases_cumulative_autochthonous = pre_average.autochthonous / num_days_in_epi_week;
      isoweek_hash[d][c].cases_cumulative_imported = pre_average.imported / num_days_in_epi_week;
    })
  })
...
function add_epi_week_composition(isow_hash, isoweek_start_date, date_country_cases_hash, date) {
  ...
  Object.keys(date_country_cases_hash).forEach(c => {
    ...
    ['autochthonous', 'imported'].forEach(kind => {
...
function map_paho(filename_date, diff_in_days, obj_current, obj_past) {
  ...
  return Object.keys(obj_current.countries).reduce((h, c) => {
    ...
    ['autochthonous', 'imported'].forEach(kind => {
      cases_cumulative = obj_current.countries[c][case_kind_lookup[kind]];

      // If country is in previous week file, assign confirmed cases to cases_old
      // so you can determine growth since last week.
      var cases_old = obj_past.countries[c] ? obj_past.countries[c][case_kind_lookup[kind]] : 0;
