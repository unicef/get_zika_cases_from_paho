...
var getFiles = function (dir, provider) {
...
exports.epi_2_iso = function(dir, provider) {
  return new Promise((resolve, reject) => {
    ...
    .then(files => {
      ...
      bluebird.reduce(ordered_dates, (h, e, i) => {
        return determine_new_cases_per_epi_week_date(e, i, ordered_dates, h, provider, dir)
      ...
      .then(daily_country_cases_hash => {
        ...
        bluebird.each(Object.keys(isoweek_hash_updated_cumulative_cases), date => {
          var file = dir + provider + '/iso/' + date + '.json';
 ...
 */
function add_epi_week_composition(isow_start_day, date_country_cases_hash,isoweek_start_date, date) {
  Object.keys(date_country_cases_hash).forEach(c => {
    ...
    var epi_cases_cumulative = country_obj.cases_cumulative;
    ...
    if (isow_start_day[c].epi_weeks) {
      isow_start_day[c].epi_cases_cumulative.push(epi_cases_cumulative);
      isow_start_day[c].epi_week_2_cases_per_day[epi_week_date] = new_cases_per_day;
      isow_start_day[c].epi_week_2_cumulative[epi_week_date] = epi_cases_cumulative;
      ...
      isow_start_day[c].epi_week_2_cases_per_day[epi_week_date] = new_cases_per_day;
    // For adding and averaging later to get balanced cumulative cases for iso week.
      isow_start_day[c].epi_cases_cumulative = [epi_cases_cumulative];
    }
  })
...
function determine_new_cases_per_epi_week_date(filename_date, i, ordered_dates, all_dates_hash, provider, dir) {
  return new Promise((resolve, reject) => {
